...
@jsonrpc.method('Log.add(category=String, level=String, message=String, timestamp=Number, expires_at=Number) -> String', validate=True)
...
@jsonrpc.method('Log.addCustom(category=String, level=String, message=String, timestamp=Number, expires_at=Number, tags=Array, additional_data=Object) -> String', validate=True)
@auth.requires_rpc_auth
def addCustomLog(category, level, message, timestamp, expires_at, tags, additional_data):
    log_id = log_db.addCustomLog(g.auth.username, category, level, message, timestamp, expires_at, tags, additional_data)
    return { "logId": str(log_id) }


@jsonrpc.method('Log.get(category=String, search_filter=Object, limit=Number, offset=Number, sort_field=String, sort_type=String) -> Array', validate=True)
@auth.requires_rpc_auth
def getLog(category, search_filter={}, limit=100, offset=0, sort_field="timestamp", sort_type="ASCENDING"): #or DESCENDING
...
def deleteLog(category, search_filter={}):
    result = log_db.removeLog(g.auth.username, category, search_filter, update_data)
